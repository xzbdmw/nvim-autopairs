...
conds.is_endwise_node = function(nodes)
    ...
    return function(opts)
        ...
        if not opts.check_endwise_ts then
            return true
        end
        if nodes == nil then
            return true
        ...
        if target ~= nil and utils.is_in_table(nodes, target:type()) then
            ...
            if (begin_target ~= begin_parent and end_target == end_parent) or (end_parent - end_target == 1) then
                return true
            end
            -- return true
        else
...
conds.is_in_range = function(callback, position)
    assert(type(callback) == "function" and type(position) == "function", "callback and position should be a function")
    return function(opts)
        ...
        assert(type(cursor) == "table" and #cursor == 2, "position should be return a table like {line, col}")
        local line = cursor[1]
        local col = cursor[2]
...
conds.is_not_in_context = function()
    return function(opts)
        local context = require("nvim-autopairs.ts-utils").get_language_tree_at_position({ utils.get_cursor() })
