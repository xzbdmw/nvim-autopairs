...
M.on_attach = function(bufnr)
    ...
    if M.config.map_bs then
        api.nvim_buf_set_keymap(bufnr, "i", "<bs>", "", { callback = M.autopairs_bs, expr = true, noremap = true })
    end
    ...
    if M.config.map_c_w then
        api.nvim_buf_set_keymap(bufnr, "i", "<c-w>", "", { callback = M.autopairs_c_w, expr = true, noremap = true })
    end
...
M.autopairs_insert = function(bufnr, char)
    ...
    for _, rule in pairs(rules) do
        if rule.start_pair and rule.is_regex and rule.key_map == "" then
            local prev_char, next_char =
                utils.text_cusor_line(new_text, col + 1, #rule.start_pair, #rule.end_pair, rule.is_regex)
            local cond_opt = {
                ts_node = M.state.ts_node,
                text = new_text,
                rule = rule,
            ...
            if utils.compare(rule.start_pair, prev_char, rule.is_regex) and rule:can_pair(cond_opt) then
                local end_pair = rule:get_end_pair(cond_opt)
            ...
            if count == 2 and index >= (#line - 2) then
                ...
                for _, rule in pairs(rules) do
                    if rule.start_pair == prev_char and char_end ~= rule.end_pair then
                        local new_text = line:sub(0, index) .. rule.end_pair .. line:sub(index + 1, #line)
